...
type (
	// Watcher is responsible for looking over MetaOS blockchain and reporting new transactions to the contract
	Watcher struct {
		urlWS    string
		urlLCD   string
		contract string
...
var (
	metaosConnectionErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_metaos_connection_errors_total",
			Help: "Total number of MetaOS connection errors",
		}, []string{"reason"})
	metaosMessagesConfirmed = promauto.NewCounter(
		prometheus.CounterOpts{
			Name: "wormhole_metaos_messages_confirmed_total",
			Help: "Total number of verified metaos messages found",
		})
	currentMetaOSHeight = promauto.NewGauge(
		prometheus.GaugeOpts{
			Name: "wormhole_metaos_current_height",
			Help: "Current metaos slot height (at default commitment level, not the level used for observations)",
		})
	queryLatency = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "wormhole_metaos_query_latency",
			Help: "Latency histogram for metaos RPC calls",
		}, []string{"operation"})
)
...
// NewWatcher creates a new MetaOS contract watcher
func NewWatcher(
	urlWS string,
	urlLCD string,
	contract string,
...
func (e *Watcher) Run(ctx context.Context) error {
	p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDMetaOS, &gossipv1.Heartbeat_Network{
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDMetaOS, 1)
		metaosConnectionErrors.WithLabelValues("websocket_dial_error").Inc()
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDMetaOS, 1)
		metaosConnectionErrors.WithLabelValues("websocket_subscription_error").Inc()
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDMetaOS, 1)
		metaosConnectionErrors.WithLabelValues("event_subscription_error").Inc()
		return fmt.Errorf("event subscription failed: %w", err)
	}
	logger.Info("subscribed to new transaction events")

	readiness.SetReady(common.ReadinessMetaOSSyncing)
	...
	go func() {
		...
		for {
			...
			// Query and report height and set currentMetaOSHeight
			...
			logger.Info("current MetaOS height", zap.Int64("block", latestBlock.Int()))
			currentMetaOSHeight.Set(float64(latestBlock.Int()))
			p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDMetaOS, &gossipv1.Heartbeat_Network{
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				return
			case r := <-e.obsvReqC:
				if vaa.ChainID(r.ChainId) != vaa.ChainIDMetaOS {
					panic("invalid chain ID")
				}

				tx := hex.EncodeToString(r.TxHash)

				logger.Info("received observation request for metaos",
				...
				if !txHashRaw.Exists() {
					logger.Error("metaos tx does not have tx hash", zap.String("payload", txJSON))
					continue
				}
				...
				if !events.Exists() {
					logger.Error("metaos tx has no events", zap.String("payload", txJSON))
					continue
				}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDMetaOS, 1)
				metaosConnectionErrors.WithLabelValues("channel_read_error").Inc()
			...
			if !txHashRaw.Exists() {
				logger.Warn("metaos message does not have tx hash", zap.String("payload", json))
				continue
			}
			txHash := txHashRaw.String()
			...
			if !events.Exists() {
				logger.Warn("metaos message has no events", zap.String("payload", json))
				continue
			}
			...
			if err != nil {
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDMetaOS, 1)
				metaosConnectionErrors.WithLabelValues("guardian_set_req_error").Inc()
			...
			logger.Debug("current guardian set on MetaOS",
...
func EventsToMessagePublications(contract string, txHash string, events []gjson.Result, logger *zap.Logger) []*common.MessagePublication {
	...
	for _, event := range events {
		if !event.IsObject() {
			logger.Warn("metaos event is invalid", zap.String("tx_hash", txHash), zap.String("event", event.String()))
			continue
		}
		...
		if !attributes.Exists() {
			logger.Warn("metaos message event has no attributes", zap.String("tx_hash", txHash), zap.String("event", event.String()))
			continue
		}
		mappedAttributes := map[string]string{}
		for _, attribute := range attributes.Array() {
			if !attribute.IsObject() {
				logger.Warn("metaos event attribute is invalid", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}
			keyBase := gjson.Get(attribute.String(), "key")
			if !keyBase.Exists() {
				logger.Warn("metaos event attribute does not have key", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}
			valueBase := gjson.Get(attribute.String(), "value")
			if !valueBase.Exists() {
				logger.Warn("metaos event attribute does not have value", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}

			key, err := base64.StdEncoding.DecodeString(keyBase.String())
			if err != nil {
				logger.Warn("metaos event key attribute is invalid", zap.String("tx_hash", txHash), zap.String("key", keyBase.String()))
				continue
			}
			value, err := base64.StdEncoding.DecodeString(valueBase.String())
			if err != nil {
				logger.Warn("metaos event value attribute is invalid", zap.String("tx_hash", txHash), zap.String("key", keyBase.String()), zap.String("value", valueBase.String()))
				continue
			}
		...
		if !ok {
			logger.Warn("metaos wasm event without contract address field set", zap.String("event", event.String()))
			continue
		}
		...
		logger.Info("new message detected on metaos",
		...
		messagePublication := &common.MessagePublication{
			...
			EmitterChain:     vaa.ChainIDMetaOS,
