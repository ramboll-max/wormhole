...
type (
	// Watcher is responsible for looking over Sophon blockchain and reporting new transactions to the contract
	Watcher struct {
		urlWS    string
		urlLCD   string
		contract string
...
var (
	sophonConnectionErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_sophon_connection_errors_total",
			Help: "Total number of Sophon connection errors",
		}, []string{"reason"})
	sophonMessagesConfirmed = promauto.NewCounter(
		prometheus.CounterOpts{
			Name: "wormhole_sophon_messages_confirmed_total",
			Help: "Total number of verified sophon messages found",
		})
	currentSophonHeight = promauto.NewGauge(
		prometheus.GaugeOpts{
			Name: "wormhole_sophon_current_height",
			Help: "Current sophon slot height (at default commitment level, not the level used for observations)",
		})
	queryLatency = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "wormhole_sophon_query_latency",
			Help: "Latency histogram for sophon RPC calls",
		}, []string{"operation"})
)
...
// NewWatcher creates a new Sophon contract watcher
func NewWatcher(
	urlWS string,
	urlLCD string,
	contract string,
...
func (e *Watcher) Run(ctx context.Context) error {
	p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDSophon, &gossipv1.Heartbeat_Network{
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSophon, 1)
		sophonConnectionErrors.WithLabelValues("websocket_dial_error").Inc()
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSophon, 1)
		sophonConnectionErrors.WithLabelValues("websocket_subscription_error").Inc()
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSophon, 1)
		sophonConnectionErrors.WithLabelValues("event_subscription_error").Inc()
		return fmt.Errorf("event subscription failed: %w", err)
	}
	logger.Info("subscribed to new transaction events")

	readiness.SetReady(common.ReadinessSophonSyncing)
	...
	go func() {
		...
		for {
			...
			// Query and report height and set currentSophonHeight
			...
			logger.Info("current Sophon height", zap.Int64("block", latestBlock.Int()))
			currentSophonHeight.Set(float64(latestBlock.Int()))
			p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDSophon, &gossipv1.Heartbeat_Network{
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				return
			case r := <-e.obsvReqC:
				if vaa.ChainID(r.ChainId) != vaa.ChainIDSophon {
					panic("invalid chain ID")
				}

				tx := hex.EncodeToString(r.TxHash)

				logger.Info("received observation request for sophon",
				...
				if !txHashRaw.Exists() {
					logger.Error("sophon tx does not have tx hash", zap.String("payload", txJSON))
					continue
				}
				...
				if !events.Exists() {
					logger.Error("sophon tx has no events", zap.String("payload", txJSON))
					continue
				}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSophon, 1)
				sophonConnectionErrors.WithLabelValues("channel_read_error").Inc()
			...
			if !events.Exists() {
				logger.Warn("sophon message has no events", zap.String("payload", json))
				continue
			}
			...
			if err != nil {
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSophon, 1)
				sophonConnectionErrors.WithLabelValues("guardian_set_req_error").Inc()
			...
			logger.Debug("current guardian set on Sophon",
...
func EventsToMessagePublications(contract string, txHash string, events []gjson.Result, logger *zap.Logger) []*common.MessagePublication {
	...
	for _, event := range events {
		if !event.IsObject() {
			logger.Warn("sophon event is invalid", zap.String("tx_hash", txHash), zap.String("event", event.String()))
			continue
		}
		...
		if !attributes.Exists() {
			logger.Warn("sophon message event has no attributes", zap.String("tx_hash", txHash), zap.String("event", event.String()))
			continue
		}
		mappedAttributes := map[string]string{}
		for _, attribute := range attributes.Array() {
			if !attribute.IsObject() {
				logger.Warn("sophon event attribute is invalid", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}
			keyBase := gjson.Get(attribute.String(), "key")
			if !keyBase.Exists() {
				logger.Warn("sophon event attribute does not have key", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}
			valueBase := gjson.Get(attribute.String(), "value")
			if !valueBase.Exists() {
				logger.Warn("sophon event attribute does not have value", zap.String("tx_hash", txHash), zap.String("attribute", attribute.String()))
				continue
			}

			key, err := base64.StdEncoding.DecodeString(keyBase.String())
			if err != nil {
				logger.Warn("sophon event key attribute is invalid", zap.String("tx_hash", txHash), zap.String("key", keyBase.String()))
				continue
			}
			value, err := base64.StdEncoding.DecodeString(valueBase.String())
			if err != nil {
				logger.Warn("sophon event value attribute is invalid", zap.String("tx_hash", txHash), zap.String("key", keyBase.String()), zap.String("value", valueBase.String()))
				continue
			}
		...
		logger.Info("new message detected on sophon",
		...
		messagePublication := &common.MessagePublication{
			...
			EmitterChain:     vaa.ChainIDSophon,
